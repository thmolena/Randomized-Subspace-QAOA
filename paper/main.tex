...
\title{\textbf{Task-Weighted Observable Subspaces\\
for Repeated Multi-Angle QAOA}}
\author{Molena Huynh\\
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small Task-Weighted Observable Subspaces for Repeated Multi-Angle QAOA}
\fancyfoot[C]{\thepage}
...
\label{tab:legacy}
\resizebox{\textwidth}{!}{\input{tables/table1_summary.tex}}
\end{table*}
...
Exact hash prefixes remain reproducible from
\texttt{tables/table\_reproduction.tex} and the display manifest.  This
integrity metadata is validated before rendering but is excluded from the
five numbered experimental tables.
