...
\hypersetup{
  pdftitle={Task-Weighted QAOA Subspaces: Local Optimality without a Matched-SPSA Query Saving},
  pdfauthor={Molena Huynh},
  pdfsubject={A matrix-free construction and matched negative resource audit},
  pdfkeywords={QAOA, randomized numerical linear algebra, active subspaces, resource accounting}
}

\title{\textbf{Task-Weighted QAOA Subspaces:\\
Local Optimality without a Matched-SPSA Query Saving}}
\author{Molena Huynh\\
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small Task-Weighted QAOA Subspaces under Matched Resource Accounting}
\fancyfoot[C]{\thepage}
...
\begin{abstract}
Repeated optimization on a fixed graph may change only the weights of
commuting edge observables.  We formulate this regime as
task-distribution-weighted dimension reduction for multi-angle QAOA.  For
observable Jacobian $\J$ and task-weight second moment
$\M=LL^{\trans}$, leading left singular vectors of $\J^{\trans}L$ minimize
expected discarded local gradient energy.  A matrix-free randomized algorithm
constructs the subspace, while a residual diagnostic governs refresh.  We
audit scalar objectives, vector observables, shots, and simulator-only
derivatives separately.  Across 16 dependent exact-statevector graph-depth
cells, residual-gated reuse changes mean approximation ratio by
$-1.57\pm1.04$ percentage points relative to matched full-space SPSA and uses
$1.218$ times as many forward circuit evaluations, failing the declared
development gate.  A separate 256-shot objective pilot remains
simulator-assisted and finds no resolved advantage.  The results establish a
local representation and a falsifiable accounting protocol, not query,
...
well defined and outperforms one unmatched random-basis control, yet the tested
...
The displayed development decision is generated from the versioned evidence
bundle.}
\label{fig:protocol}
\end{figure*}

Our contributions are fourfold.  First, we prove the exact local optimality criterion
for task-weighted local subspaces.  Second, we give a matrix-free construction
and an explicit resource ledger for the stated simulator access model.  Third,
we distinguish reusable geometry from
...
rendered deterministically from versioned configuration metadata.
...
task-index performance for the exact track, which has one evaluation
realization per graph-depth cell.  The right panel shows that residual gating
refreshes on most eligible transitions, limiting amortization in the tested
regime.}
\label{fig:streams}
\end{figure*}
...
training instances \citep{khairy2020learning}.  Optimizer rankings can depend
on budget, noise, and objective geometry \citep{bonet2023performance}, while a
linear-ramp protocol supplies a low-complexity schedule comparator
\citep{montanez2025ramp}.  A complementary graph-conditioned trust-region
method transfers a learned Gaussian search policy across graph instances under
an adaptive query budget \citep{huynh2026gctr}.  Here the graph and circuit are
fixed: the changing object is the weight vector, and the learned object is a
local parameter subspace rather than a graph-conditioned search distribution.
...
\citep{audet2006mesh}, and covariance-matrix adaptation learns a search
distribution \citep{hansen2001cma}.  These methods alter the search strategy,
whereas \RSQ fixes the optimizer family and changes only its coordinate
subspace.  Disjoint training and evaluation task streams reduce, but do not
remove, selection bias from development on the same topology grid
\citep{cawley2010overfitting}.

\subsection{Limits of Static and Unweighted Subspaces}

\subsubsection{Inconsistency under Task Shift}
...
Statistical uncertainty summaries cannot repair an omitted physical direction
after the fact.  Sampling uncertainty and representation loss are distinct
questions and require distinct assumptions.

\subsubsection{Refresh Bias and Hidden Physical Cost}
...
\subsubsection{No Scalar-Query Saving under Matched SPSA}
...
\subsection{Task-Weighted Observable Subspaces}

\subsubsection{Optimal Task-Weighted Subspace}
...
\subsubsection{Matrix-Free Construction}
...
\subsubsection{Residual-Gated Refresh}
...
geometry, but it is not matched to the learned methods in realized rank or
construction cost.  It therefore provides only a limited geometry check.
Fixed and random refresh controls separate timing from content.

Multiple diagnostic comparisons are descriptive.  We do not retrofit post-hoc
inference after examining the development grid.  The only decision is the
stated three-part development gate.

\subsubsection{Matched Full and Reduced SPSA}
...
\subsubsection{Exact-Statevector Repeated-Task Benchmark}
...
\subsubsection{Matched Development Audit}
...
$-1.57\pm1.04$ points and a cost ratio of $1.218$.  It exceeds the unmatched
random-basis control but fails the quality condition.  Per-task refresh gives
...
shots, while subspace construction uses exact simulator observables and VJPs.
Reported approximation ratios are exact post-evaluations of parameters selected
with sampled objectives.  This is not an end-to-end hardware experiment.}
\label{fig:shotaudit}
\end{figure*}

\subsubsection{Finite-Shot Pilot}
...
The experiment supports three bounded conclusions.  First, the learned basis
outperforms one unmatched random-basis control, but this does not isolate task
weighting from rank or construction cost.  Second, frequent refresh is one
observed obstacle to amortization on these short streams, not a demonstrated
dominant cause.  Third, the simulator-assisted construction cannot support a
hardware-efficiency claim.  Longer task banks, larger graphs, calibrated
refresh decisions, and device-native sensitivity actions remain untested.
...
A prospective design file proposes 40 new topology units, longer task streams,
changepoints, independent-weight stress tests, shot and readout perturbations,
device-native coordinate gradients, and dense-oracle controls.  It is
incomplete, unregistered, unpowered for its censored primary endpoint, and
unexecuted; it supplies no confirmatory evidence in this paper.  Its analysis,
sample size, implementation, and decision rule must be frozen before any
outcomes are generated or inspected.  Distribution-free calibration is
attractive only when its exchangeability conditions are defensible
\citep{angelopoulos2021gentle,barber2022exchangeability}.

The installable package exposes \texttt{rsqaoa-experiment} for a direct run and
\texttt{rsqaoa-reproduce} for display regeneration or frozen-evidence
validation.  Optimization reruns require a source environment matching the
recorded protocol; the command does not refreeze an existing evidence record.
Versioned source records, protocol hashes, implementation hashes, and
generated-asset hashes make the displayed values traceable without implying
that deterministic rerunning removes modeling uncertainty.

\section{Methods}

The exact and finite-shot development protocols use the fixed configurations,
seed rules, SPSA schedules, and resource-ledger definitions reported in the
Supplementary Methods below.  Displays are regenerated only after source-data
and summary hashes are validated.  The prospective confirmatory design is
distributed for inspection but is not an executed method in this study.
Extended Data Figures~\ref{fig:operatorbudget}--\ref{fig:reproductionmap}
and Extended Data Tables~\ref{tab:development-gate}--\ref{tab:shot} report
the complete operator ledger, protocol details, sensitivity analyses, and
reproduction chain.

\subsection{Use of generative AI tools}

OpenAI Codex was used to assist with code and manuscript editing, automated
checks, and formatting.  The author reviewed every generated or modified
artifact and takes responsibility for the study design, calculations,
interpretation, and text.  No large language model is listed as an author.

\section*{Data availability}
The source data underlying the displayed results are included in
\texttt{experiments/results} in the submission package.  A public versioned
release and archival DOI are pending; this manuscript does not claim that a
public or permanent archive is already available.  The study uses only
synthetic graph instances and has no restricted or third-party data.

\section*{Code availability}
The implementation, frozen development configurations, validation scripts, and
reproduction commands are supplied for peer review in the submission package.
A public versioned repository release and archival DOI are pending; this
manuscript makes no claim of existing public or permanent code availability.

\section*{Author contributions}
M.H. conceived the study, developed the theory and software, performed the
experiments and analysis, prepared the displays, and wrote the manuscript.
...
\renewcommand{\theHfigure}{extended-data.figure.\arabic{figure}}
\renewcommand{\theHtable}{extended-data.table.\arabic{table}}
...
\begin{algorithm}{Matrix-free task-weighted QB}
\label{alg:matrix-free-qb}
...
\resizebox{\textwidth}{!}{\input{tables/table_development_gate.tex}}
\end{table*}
...
\label{tab:protocol}
\resizebox{\textwidth}{!}{\input{tables/table_protocol.tex}}
\end{table*}
...
\texttt{experiments/results/maxcut\_small.csv},
\texttt{experiments/results/amortized\_development.csv}, and
\texttt{experiments/results/amortized\_shot\_development.csv}, together with
...
The incomplete prospective design proposes changepoints, independent task
distributions, longer streams, larger graphs, readout perturbations, and
device-native coordinate gradients.  It is unregistered, unexecuted, and not
evidence for the present claims.  Classical-shadow ideas may reduce the cost
of broad observable estimation in other operator families
\citep{huang2020shadows}; they are unnecessary for the commuting MaxCut
